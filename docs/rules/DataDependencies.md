...
* 値: 各データの詳細を定義する辞書.
    ...
    * `columns` (任意): データが `table` 形式の場合に列情報を定義するリスト. `format` が `table` の場合は必須. 以下の要素を持つ辞書のリスト.
        * `name` (必須): 列名 (文字列).
    ...
    * `keys` (任意): データが `dictionary` 形式の場合にキー情報を定義するリスト. `format` が `dictionary` の場合は必須. 以下の要素を持つ辞書のリスト.
        * `name` (必須): キー名 (文字列).
